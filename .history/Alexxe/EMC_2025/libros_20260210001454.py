...
def escribirInforme(autor, total, devueltos, pendientes, retrasados, libro_mayor_solicitado):

  with open(f"informe_{autor}.txt", "w", encoding="UTF-8") as fichero:
    fichero.write(f"Autor: {autor}\n")
    fichero.write(f"Total de libros préstamos: {total}\n")
    fichero.write(f"Total de libros devueltos: {devueltos}\n")
    fichero.write(f"Total de libros pendientes: {pendientes}\n")
    fichero.write(f"Total de libros retrasados: {retrasados}\n")
    fichero.write(f"Libro más solicitado: {libro_mayor_solicitado}\n")
#-----------------------------------------------------+
