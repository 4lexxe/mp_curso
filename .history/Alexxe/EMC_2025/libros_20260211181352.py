...
#===========================================================
def obtenerLibroMasSolicitador(libros_lista):
  libro_mayor = ""
  libro_max = 0
...
def escribirInforme(autor, total, devueltos, pendientes, retrasados, libro_top):
  with open(f"informe_{autor}", "w", encoding="UTF-8") as autor:
    autor.write(f"Autor: {autor}\n")
    autor.write(f"Total de libros préstamos: {total}")
    autor.write(f"Total de libros devueltos: {devueltos}")
    autor.write(f"Total de libros pendientes: {pendientes}")
    autor.write(f"Total de libros retrasados: {retrasados}")
    autor.write(f"Libro más solicitado: {libro_top}")

#==================PRINCIPAL========================

lista = leerPrestamos()
listarPorFecha(lista, "2025", "09")
contarSolicitudesAutor(lista, "Julio Cortázar")
